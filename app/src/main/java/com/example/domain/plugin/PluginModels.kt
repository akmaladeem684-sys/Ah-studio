...
enum class PluginCategory(val key: String, val displayName: String, val iconName: String) {
  ...
  companion object {
    fun fromKey(key: String): PluginCategory {
      ...
      return when {
        normalized in listOf("font", "fonts", "typography", "urdu_font", "english_font", "calligraphy") -> FONT
        ...
        normalized in listOf("filter", "filters", "lut", "luts", "color_grade", "color_grading") -> FILTER
...
data class PluginItemManifest(
  ...
  val itemCategory: PluginCategory get() = if (categoryKey.isNotBlank()) PluginCategory.fromKey(categoryKey) else PluginCategory.OTHER
